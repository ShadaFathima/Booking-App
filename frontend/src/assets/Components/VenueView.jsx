  ...
  const VenueView = ({ isLoggedIn }) => {
    ...
    const handleCheckAvailability = async () => {
      try {
        const response = await axios.post('http://127.0.0.1:8000/api/check-availability/', {
          venueTitle: venue.title,
          selectedDate: startDate,
          selectedTimeSlot: selectedTimeSlot
        });
        const responseData = response.data;
        if (responseData.available !== undefined) { // Check if the 'available' field is present in the response
          if (responseData.available) {
            alert('Venue is available for the selected date and time slot. You can proceed with booking.');
            // Proceed with booking logic here
          } else {
            alert('Venue is not available for the selected date and time slot. Please choose another date or time slot.');
          }
          setAvailabilityChecked(true); // Set availabilityChecked state after checking availability
        } else {
